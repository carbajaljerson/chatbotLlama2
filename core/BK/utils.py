...
def ingest():
    ...
    loader = PyPDFLoader('./media/PresentaciónLinkedIn.pdf')
    data = loader.load()
...
def pineconeLogin():
    ...
    pinecone.init(
        api_key=constant.PINECONE_API_KEY,
        environment=constant.PINECONE_API_ENV
        )
...
def createEmbeddings(EMBEDDING_MODEL):
    ...
    return embeddings
#def readVectorStore(docs, INDEX_NAME, embeddings):
def readVectorStore(INDEX_NAME, embeddings):
    ...
    #return Pinecone.from_texts([t.page_content for t in docs], embeddings, index_name=INDEX_NAME)
